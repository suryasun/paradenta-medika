# task-157: Create and List Expense (POST/GET /finance/expenses)
...
Implement expense creation (draft) and listing.
...
- Presentation layer: route, controller, DTO/validator for `GET/POST /finance/expenses`.
- Application layer: `CreateExpenseUseCase / ListExpensesUseCase`.
...
Adds GET/POST /finance/expenses per docs/03-sad/17-module-finance.md Section 6.3.
...
- `CreateExpenseUseCase / ListExpensesUseCase`, route, controller, tests

## Acceptance Criteria

Per docs/01-prd/acceptance-criteria/finance.md (sourced from docs/03-sad/17-module-finance.md):

- Create succeeds in `draft` status; List supports filter by branch, category, status, date range.
