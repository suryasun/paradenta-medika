# task-152: Void Journal (POST /finance/journals/{journalId}/void)
...
Void a journal that must never have been posted (draft-only voiding) — distinct from Reverse, which targets posted journals.
...
- Presentation layer: route, controller, DTO/validator for `POST /finance/journals/{journalId}/void`.
- Application layer: `VoidJournalUseCase`.
...
Adds POST /finance/journals/{journalId}/void per docs/03-sad/17-module-finance.md Section 6.2.
...
- `VoidJournalUseCase`, route, controller, tests

## Acceptance Criteria

Per docs/01-prd/acceptance-criteria/finance.md (sourced from docs/03-sad/17-module-finance.md):

- Void rejected on an already-posted journal (must use Reverse instead).
