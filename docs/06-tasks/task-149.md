# task-149: Update Journal (PATCH /finance/journals/{journalId})
...
Allow a draft journal's lines/description to be corrected before posting.
...
- Presentation layer: route, controller, DTO/validator for `PATCH /finance/journals/{journalId}`.
- Application layer: `UpdateJournalUseCase`.
...
Adds PATCH /finance/journals/{journalId} per docs/03-sad/17-module-finance.md Section 6.2.
...
- `UpdateJournalUseCase`, route, controller, tests

## Acceptance Criteria

Per docs/01-prd/acceptance-criteria/finance.md (sourced from docs/03-sad/17-module-finance.md):

- Update rejected once journal is `posted`/`reversed`/`voided` (must be draft).
