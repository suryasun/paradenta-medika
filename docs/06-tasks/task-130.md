# task-130: Start Count (POST /stock-opnames/{opnameId}/start-count)
...
Freeze the system quantity snapshot and transition the opname to `counting`.
...
- Presentation layer: route, controller, DTO/validator for `POST /warehouse/stock-opnames/{opnameId}/start-count`.
- Application layer: `StartStockOpnameCountUseCase`.
...
Adds POST /warehouse/stock-opnames/{opnameId}/start-count per docs/03-sad/18-module-warehouse.md Section 6.4.
...
- `StartStockOpnameCountUseCase`, route, controller, tests

## Acceptance Criteria

Per docs/01-prd/acceptance-criteria/warehouse.md (sourced from docs/03-sad/18-module-warehouse.md):

- System quantity snapshot is immutable once counting starts.
