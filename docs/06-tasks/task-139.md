# task-139: Movements Report (GET /warehouse/reports/movements)
...
Implement `GetMovementsReportUseCase` exposing `GET /warehouse/reports/movements` per docs/03-sad/18-module-warehouse.md Section 6.5 Reports, so Warehouse Staff can review in/out by type, reference, actor.
...
- Presentation layer: route, controller, query DTO/validator for `GET /warehouse/reports/movements`.
- Application layer: `GetMovementsReportUseCase`, read-only, sourced from warehouse_stock_ledger.
...
Adds GET /warehouse/reports/movements.
...
- `GetMovementsReportUseCase`, route, controller, tests

## Acceptance Criteria

Per docs/01-prd/acceptance-criteria/warehouse.md (sourced from docs/03-sad/18-module-warehouse.md):

- Output matches the In/out by type, reference, actor shape defined in Section 6.5.
