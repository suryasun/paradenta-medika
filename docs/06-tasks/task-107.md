# task-107: Submit Purchase Order (POST /purchase-orders/{purchaseOrderId}/submit)
...
Transition a draft PO to `submitted`, per UC-WHS-001 step 1.
...
- Presentation layer: route, controller, DTO/validator for `POST /warehouse/purchase-orders/{purchaseOrderId}/submit`.
- Application layer: `SubmitPurchaseOrderUseCase`, gated by `procurement submit permission`.
...
Adds POST /warehouse/purchase-orders/{purchaseOrderId}/submit per docs/03-sad/18-module-warehouse.md Section 6.2.
...
Gated by procurement submit permission. Audit Trail entry required. Maker-checker enforced where applicable (approver ≠ requester).
...
- `SubmitPurchaseOrderUseCase`, route, controller, DTOs, tests
...
- Submission is idempotent for the same Idempotency-Key.
