# task-148: List and Get Journal (GET /finance/journals, GET /finance/journals/{journalId})
...
Implement journal listing/detail retrieval, gated by finance.journal.read.
...
- Presentation layer: route, controller, DTO/validator for `GET /finance/journals, GET /finance/journals/{journalId}`.
- Application layer: `ListJournalsUseCase / GetJournalUseCase`.
...
Adds GET /finance/journals, GET /finance/journals/{journalId} per docs/03-sad/17-module-finance.md Section 6.2.
...
- `ListJournalsUseCase / GetJournalUseCase`, route, controller, tests

## Acceptance Criteria

Per docs/01-prd/acceptance-criteria/finance.md (sourced from docs/03-sad/17-module-finance.md):

- List supports pagination/filter by branch, date range, status, account.
