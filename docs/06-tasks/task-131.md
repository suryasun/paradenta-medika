# task-131: Submit Stock Opname (POST /stock-opnames/{opnameId}/submit)
...
Submit counted quantities for approval, computing variance per line.
...
- Presentation layer: route, controller, DTO/validator for `POST /warehouse/stock-opnames/{opnameId}/submit`.
- Application layer: `SubmitStockOpnameUseCase`.
...
Adds POST /warehouse/stock-opnames/{opnameId}/submit per docs/03-sad/18-module-warehouse.md Section 6.4.
...
- `SubmitStockOpnameUseCase`, route, controller, tests

## Acceptance Criteria

Per docs/01-prd/acceptance-criteria/warehouse.md (sourced from docs/03-sad/18-module-warehouse.md):

- Variance (counted − system) computed and stored per line.
