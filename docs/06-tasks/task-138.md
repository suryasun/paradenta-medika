# task-138: Stock Balance Report (GET /warehouse/reports/stock-balance)
...
Implement `GetStockBalanceReportUseCase` exposing `GET /warehouse/reports/stock-balance` per docs/03-sad/18-module-warehouse.md Section 6.5 Reports, so Warehouse Staff can review current, reserved, available, minimum, status.
...
- Presentation layer: route, controller, query DTO/validator for `GET /warehouse/reports/stock-balance`.
- Application layer: `GetStockBalanceReportUseCase`, read-only, sourced from warehouse_stocks.
...
Adds GET /warehouse/reports/stock-balance.
...
- `GetStockBalanceReportUseCase`, route, controller, tests

## Acceptance Criteria

Per docs/01-prd/acceptance-criteria/warehouse.md (sourced from docs/03-sad/18-module-warehouse.md):

- Output matches the Current, reserved, available, minimum, status shape defined in Section 6.5.
