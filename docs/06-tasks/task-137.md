# task-137: Stock Card Report (GET /warehouse/reports/stock-card)
...
Implement `GetStockCardReportUseCase` exposing `GET /warehouse/reports/stock-card` per docs/03-sad/18-module-warehouse.md Section 6.5 Reports, so Warehouse Staff can review ledger per item/warehouse/batch.
...
- Presentation layer: route, controller, query DTO/validator for `GET /warehouse/reports/stock-card`.
- Application layer: `GetStockCardReportUseCase`, read-only, sourced from warehouse_stock_ledger.
...
Adds GET /warehouse/reports/stock-card.
...
- `GetStockCardReportUseCase`, route, controller, tests

## Acceptance Criteria

Per docs/01-prd/acceptance-criteria/warehouse.md (sourced from docs/03-sad/18-module-warehouse.md):

- Output matches the Ledger per item/warehouse/batch shape defined in Section 6.5.
