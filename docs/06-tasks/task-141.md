# task-141: Expiry Report (GET /warehouse/reports/expiry)
...
Implement `GetExpiryReportUseCase` exposing `GET /warehouse/reports/expiry` per docs/03-sad/18-module-warehouse.md Section 6.5 Reports, so Warehouse Staff can review near-expiry, expired, quarantine stock.
...
- Presentation layer: route, controller, query DTO/validator for `GET /warehouse/reports/expiry`.
- Application layer: `GetExpiryReportUseCase`, read-only, sourced from warehouse_batches.
...
Adds GET /warehouse/reports/expiry.
...
- `GetExpiryReportUseCase`, route, controller, tests

## Acceptance Criteria

Per docs/01-prd/acceptance-criteria/warehouse.md (sourced from docs/03-sad/18-module-warehouse.md):

- Output matches the Near-expiry, expired, quarantine stock shape defined in Section 6.5.
