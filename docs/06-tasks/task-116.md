...
Implement the first step of UC-WHS-004: create a transfer draft between two warehouses.
...
- Presentation layer: route, controller, DTO/validator for `POST /warehouse/transfers`.
- Application layer: `CreateStockTransferUseCase`.
...
Adds POST /warehouse/transfers per docs/03-sad/18-module-warehouse.md Section 6.3.
...
- `CreateStockTransferUseCase`, route, controller, tests

## Acceptance Criteria

Per docs/01-prd/acceptance-criteria/warehouse.md (sourced from docs/03-sad/18-module-warehouse.md):

- Rejects sourceWarehouseId == destinationWarehouseId with `WHS_SOURCE_DESTINATION_SAME`.
