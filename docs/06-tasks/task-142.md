# task-142: Opnames Report (GET /warehouse/reports/opnames)
...
Implement `GetOpnamesReportUseCase` exposing `GET /warehouse/reports/opnames` per docs/03-sad/18-module-warehouse.md Section 6.5 Reports, so Warehouse Staff can review snapshot, count, variance, approval.

## Depends On

- task-102
- task-103
- task-127 (Stock Opname)
...
- Presentation layer: route, controller, query DTO/validator for `GET /warehouse/reports/opnames`.
- Application layer: `GetOpnamesReportUseCase`, read-only, sourced from warehouse_stock_opnames.
...
Adds GET /warehouse/reports/opnames.
...
- `GetOpnamesReportUseCase`, route, controller, tests

## Acceptance Criteria

Per docs/01-prd/acceptance-criteria/warehouse.md (sourced from docs/03-sad/18-module-warehouse.md):

- Output matches the Snapshot, count, variance, approval shape defined in Section 6.5.
