...
1. **Reservations by Patient Type Report** (task-300): new page, `/reports/by-patient-type` — same date-preset shell as §10's Completed Reservations Report (renamed to Reservation By Status Report in §12), two summary cards (New count/%, Old count/%), a `TrendChart` used categorically (New vs. Old, not a date trend), results table. Distinct from §8.4's New Patient Report, which only ever shows NEW-tagged rows.
...
## 12. Reservation Module Addendum #4 (task-305–310, Post-Roadmap Addendum)

Three more changes on top of §10/§11's three reservation reports:

1. **Reservation By Status Report** (task-305): `/reports/completed-reservations` → `/reports/by-status` (nav label "Reservation By Status"). Same shell as §10's Completed Reservations Report, plus a Status `<Select>` (All Statuses + the 8 `ReservationStatus` values — reused from `ReservationListView.tsx`'s existing `RESERVATION_STATUS_TONE` map, not a new label component) defaulting to `COMPLETED` so the report's original default view is unchanged, and a Day/Month `<Select>` next to it. The summary card label and existing trend chart both update to reflect the selected status/granularity.
2. **Status filter on By Patient Type / By Doctor** (task-306/307): §11 items 1–2 both gain the same Status `<Select>` as above (default "All Statuses", unlike By Status's `COMPLETED` default) — narrows both the results table and each report's summary/chart.
3. **Day/Month trend section** (task-308/309/310): all three reports gain/generalize a Day/Month `<Select>`. By Status's existing trend chart is generalized to respect it; By Patient Type and By Doctor each gain a new, separate "Trend Over Time" `Card`+`TrendChart` section below their existing comparison chart (New vs. Old / per-doctor) — additive, not a replacement.
