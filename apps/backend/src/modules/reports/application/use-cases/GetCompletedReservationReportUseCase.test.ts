import { GetCompletedReservationReportUseCase } from './GetCompletedReservationReportUseCase';
...
describe('GetCompletedReservationReportUseCase', () => {
  it('includes only COMPLETED reservations within the given range, excluding other statuses and out-of-range rows', async () => {
    const reservationRepository = new FakeReservationRepository();
    const useCase = new GetCompletedReservationReportUseCase(reservationRepository);
  ...
  it('builds a day-by-day trend of completed reservations for the chart', async () => {
    const reservationRepository = new FakeReservationRepository();
    const useCase = new GetCompletedReservationReportUseCase(reservationRepository);
    ...
    expect(result.summary.totalCompleted).toBe(3);
