import { GetReservationByPatientTypeReportUseCase } from './GetReservationByPatientTypeReportUseCase';
import { FakeReservationRepository } from '../../../../../tests/fakes/reservationFakes';
import { parseTimeToDate } from '../../../reservation/application/services/timeUtils';
import { ReportFilterInvalidException } from '../../domain/exceptions/ReportExceptions';

async function seedReservation(
  repo: FakeReservationRepository,
  overrides: { reservationNo: string; patientId: string; reservationDate: Date; patientTypeAtBooking: 'NEW' | 'OLD' },
) {
  return repo.create({
    reservationNo: overrides.reservationNo,
    patientId: overrides.patientId,
    doctorId: 'd1',
    branchId: 'b1',
    reservationDate: overrides.reservationDate,
    reservationTime: parseTimeToDate('09:00'),
    reservationType: 'APPOINTMENT',
    source: 'PHONE',
    patientTypeAtBooking: overrides.patientTypeAtBooking,
    createdBy: 'staff-1',
  });
}

const IN_RANGE = new Date('2026-03-10T00:00:00.000Z');
const OUT_OF_RANGE = new Date('2026-04-10T00:00:00.000Z');

// docs/06-tasks/task-300.md (Reservation Module Addendum #3)
describe('GetReservationByPatientTypeReportUseCase', () => {
  it('includes both NEW and OLD reservations within the range in items, excluding out-of-range rows', async () => {
    const reservationRepository = new FakeReservationRepository();
    const useCase = new GetReservationByPatientTypeReportUseCase(reservationRepository);

    await seedReservation(reservationRepository, { reservationNo: 'RSV-1', patientId: 'p1', reservationDate: IN_RANGE, patientTypeAtBooking: 'NEW' });
    await seedReservation(reservationRepository, { reservationNo: 'RSV-2', patientId: 'p2', reservationDate: IN_RANGE, patientTypeAtBooking: 'OLD' });
    await seedReservation(reservationRepository, { reservationNo: 'RSV-3', patientId: 'p3', reservationDate: OUT_OF_RANGE, patientTypeAtBooking: 'NEW' });

    const result = await useCase.execute({ dateFrom: '2026-03-01', dateTo: '2026-03-31', page: 1, limit: 20, sort: 'createdAt', order: 'desc' });

    expect(result.total).toBe(2);
    expect(result.items.map((i) => i.reservationNumber).sort()).toEqual(['RSV-1', 'RSV-2']);
  });

  it('computes NEW/OLD counts and percentages over the full range, not just the current page', async () => {
    const reservationRepository = new FakeReservationRepository();
    const useCase = new GetReservationByPatientTypeReportUseCase(reservationRepository);

    await seedReservation(reservationRepository, { reservationNo: 'RSV-1', patientId: 'p1', reservationDate: IN_RANGE, patientTypeAtBooking: 'NEW' });
    await seedReservation(reservationRepository, { reservationNo: 'RSV-2', patientId: 'p2', reservationDate: IN_RANGE, patientTypeAtBooking: 'NEW' });
    await seedReservation(reservationRepository, { reservationNo: 'RSV-3', patientId: 'p3', reservationDate: IN_RANGE, patientTypeAtBooking: 'OLD' });

    const result = await useCase.execute({ dateFrom: '2026-03-01', dateTo: '2026-03-31', page: 1, limit: 1, sort: 'createdAt', order: 'desc' });

    expect(result.items).toHaveLength(1);
    expect(result.summary).toEqual({
      newCount: 2,
      oldCount: 1,
      newPercentage: 66.67,
      oldPercentage: 33.33,
      breakdown: [
        { type: 'NEW', count: 2 },
        { type: 'OLD', count: 1 },
      ],
      trend: [{ date: '2026-03-10', count: 3 }],
    });
  });

  it('narrows both the table and the NEW/OLD comparison by an explicit status filter', async () => {
    const reservationRepository = new FakeReservationRepository();
    const useCase = new GetReservationByPatientTypeReportUseCase(reservationRepository);

    const r1 = await seedReservation(reservationRepository, {
      reservationNo: 'RSV-1',
      patientId: 'p1',
      reservationDate: IN_RANGE,
      patientTypeAtBooking: 'NEW',
    });
    reservationRepository.reservations.get(r1.id)!.status = 'CANCELLED';
    await seedReservation(reservationRepository, { reservationNo: 'RSV-2', patientId: 'p2', reservationDate: IN_RANGE, patientTypeAtBooking: 'OLD' });

    const result = await useCase.execute({
      dateFrom: '2026-03-01',
      dateTo: '2026-03-31',
      status: 'CANCELLED',
      page: 1,
      limit: 20,
      sort: 'createdAt',
      order: 'desc',
    });

    expect(result.total).toBe(1);
    expect(result.summary.newCount).toBe(1);
    expect(result.summary.oldCount).toBe(0);
  });

  it('builds a month-by-month trend when groupBy=month', async () => {
    const reservationRepository = new FakeReservationRepository();
    const useCase = new GetReservationByPatientTypeReportUseCase(reservationRepository);

    await seedReservation(reservationRepository, { reservationNo: 'RSV-1', patientId: 'p1', reservationDate: IN_RANGE, patientTypeAtBooking: 'NEW' });
    await seedReservation(reservationRepository, { reservationNo: 'RSV-2', patientId: 'p2', reservationDate: OUT_OF_RANGE, patientTypeAtBooking: 'OLD' });

    const result = await useCase.execute({
      dateFrom: '2026-03-01',
      dateTo: '2026-04-30',
      groupBy: 'month',
      page: 1,
      limit: 20,
      sort: 'createdAt',
      order: 'desc',
    });

    expect(result.summary.trend).toEqual([
      { date: '2026-03', count: 1 },
      { date: '2026-04', count: 1 },
    ]);
  });

  it('rejects dateFrom after dateTo', async () => {
    const reservationRepository = new FakeReservationRepository();
    const useCase = new GetReservationByPatientTypeReportUseCase(reservationRepository);

    await expect(
      useCase.execute({ dateFrom: '2026-03-31', dateTo: '2026-03-01', page: 1, limit: 20, sort: 'createdAt', order: 'desc' }),
    ).rejects.toBeInstanceOf(ReportFilterInvalidException);
  });
});
