...
import { usePermissions, useRolePermissions } from "../hooks/useRoles";
import { Permission, Role } from "../types/system.types";

// docs/02-design/pages/system.md §5's flagged pre-population bug is fixed:
// GET /system/roles/:roleId/permissions (added post-launch, see
// docs/03-sad/21-module-system.md Section 6.1 addendum) now returns the
// role's current grants, so `selected` initializes from that response
// instead of opening blank.
export function RolePermissionsModal({ role, onClose }: { role: Role; onClose: () => void }) {
  const { data: permissionsData, isLoading: permissionsLoading } = usePermissions();
  const { data: currentPermissions, isLoading: currentLoading, isError, error } = useRolePermissions(role.id);
  ...
  if (currentPermissions && currentPermissions !== syncedPermissions) {
    setSyncedPermissions(currentPermissions);
    setSelected(currentPermissions.map((p) => p.id));
  }
  ...
  const isLoading = permissionsLoading || currentLoading;
