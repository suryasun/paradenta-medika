...
// docs/06-tasks/task-301.md (Reservation Module Addendum #3). Same
// date-preset shell as the other Reservation-module reports, plus an
...
export function ReservationByDoctorReportPage() {
  ...
  const chartData = data?.summary.breakdown.map((entry) => ({ ...entry, doctorName: doctorName(entry.doctorId) })) ?? [];

  return (
    <div className="flex flex-col gap-4">
      <h1 className="text-foreground">Reservations by Doctor</h1>

      <div className="flex flex-wrap items-end gap-3 rounded-lg border border-border p-4">
        <Select
          id="by-doctor-report-preset"
          label="Preset"
          value=""
      ...
      {!isLoading && !isError && data && (
        <>
          <Card className="flex flex-col gap-1">
            <span className="text-sm text-muted">Total Doctors</span>
            <span className="font-tabular text-2xl font-semibold text-foreground">{data.summary.totalDoctors.toLocaleString()}</span>
          </Card>
